...
void procura (pLista lista, char chave[], pLista *ant, pLista *actual){
    *ant = lista; *actual = lista->prox;
    while ((*actual) != NULL && (strcmp((*actual)->itemLista.myString, chave) < 0)){
        *ant = *actual;
        *actual = (*actual)->prox;
    }
    if ((*actual) != NULL && !(strcmp((*actual)->itemLista.myString, chave)))
        *actual = NULL;
}
...
void insere (pLista lista, struct item p1){
    ...
    if (no != NULL){
        no->itemLista = p1;
        procura (lista, p1.myString, &ant, &inutil);
        no->prox = ant->prox;
        ant->prox = no;
    }
}
...
void insereNoDuplicado (pLista lista, struct item p1){
    ...
    if (no != NULL){
        no->itemLista = p1;
        procura (lista, p1.myString, &ant, &inutil);
        if (strcmp(ant->prox->itemLista.myString, p1.myString) != 0) {
            no->prox = ant->prox;
            ant->prox = no;
        }
    }
...
void imprime (pLista lista){
    ...
    while (aux){
        printf("%s %d\n", aux->itemLista.myString, aux->itemLista.num);
        aux = aux->prox;
    }
}
