// Solution 1 - a handler function to each signal
...
int main() {
  signal(SIGINT,SIG_IGN);   // Ignores the SIGINT (CTRL-C)
  signal(SIGTSTP, sigtstp); // Redirects CTRL-Z
  ...
  while (1) {
	  ...
	  sleep(2);
	  signal(SIGINT, sigint); //redirects the SIGINT again (CTRL-C) in order to be able to play the game
