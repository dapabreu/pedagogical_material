...
// #2
//IMPROVEMENT: Added missing destroy calls
    pthread_cond_destroy (&c_empty);
    pthread_mutex_destroy (&lock_empty);
    pthread_cond_destroy (&c_full);
    pthread_mutex_destroy (&lock_full);
    pthread_mutex_destroy (&lock_write); // Fix resource leak
    pthread_mutex_destroy (&lock_read);  // Fix resource leak
...
// #3
        ...
        if (produced_items>=MAX_PRODUCED_ITEMS) {
            ...
            //IMPROVEMENT: Wake up consumers to check exit condition
...
// End of #4
    ...
    free(regist); // Free allocated memory
    pthread_exit (NULL);
}

void * consumer (void * id_ptr){
    int id = *((int *)id_ptr);
    //Removed local 'int readpos = 0;' --> The global 'readpos' will be used now.
    slot regist;
    while (1){
...
// #5
        ...
        if (consumed_items>=MAX_PRODUCED_ITEMS) {
            ...
            //IMPROVEMENT: Wake up producers to check exit condition
            pthread_mutex_lock(&lock_empty);
...
// End of #5
        ...
        readpos=(readpos+1)%WAIT_LIST; // Now correctly increments the global variable
